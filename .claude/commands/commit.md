---
description: Create a Conventional Commits commit for whisper-transcribe
argument-hint: [optional focus, e.g. "just the theme files"]
---
...
3. Stage only files relevant to the requested change (`git add .` when the
   user asks to commit all changes). If the user gave a focus below, stage
   only what matches it: $ARGUMENTS
